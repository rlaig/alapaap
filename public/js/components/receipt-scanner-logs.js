...
const ReceiptScannerLogsComponent = (() => {
      ...
      <div id="rsl-tab-status" class="bt-tab-content active">
        <div class="panel">
          ...
          <div class="panel-body">
            ...
            <div id="rsl-usage" class="text-dim mt-8 fs-md"></div>
          </div>
        </div>
        ...
        <div class="panel">
          ...
          <div class="panel-body">
            ...
            <div class="flex items-center gap-8 mb-8">
              ...
              <span id="rsl-deploy-status" class="text-dim fs-md">idle</span>
            </div>
          </div>
        </div>
        <div class="panel">
  ...
  function renderOverview() {
    ...
    el.innerHTML = `
      ...
      <div id="rsl-authdb-vacuum-area" class="mt-8 hidden">
        ...
        <div id="rsl-authdb-vacuum-result" class="mt-8 fs-md"></div>
      </div>`;
  ...
  function renderUsersTable() {
    ...
    el.innerHTML = `
      ...
      <div id="rsl-adb-create-area" class="hidden" style="margin-bottom:12px;padding:8px 12px;border:1px solid var(--border)">
        <div class="form-group"><label class="form-label">email</label><input type="email" class="form-input fs-md" id="rsl-adb-new-email"></div>
        <div class="form-group"><label class="form-label">name</label><input type="text" class="form-input fs-md" id="rsl-adb-new-name"></div>
        <div class="form-group"><label class="form-label">tier</label><select class="form-input fs-md" id="rsl-adb-new-tier"><option value="free">free</option><option value="pro">pro</option><option value="admin">admin</option></select></div>
      ...
      <div class="scroll-x">
        <table class="table-console">
          ...
          ${users.map(u => {
            ...
            <td><span class="${subClass} fs-sm">${typeof subStatus === 'string' && !subStatus.includes('<') ? esc(subStatus) : subStatus}</span></td>
            <td>${activeBadge}</td>
            <td class="text-dim fs-sm">${esc(u.last_login_at || '--')}</td>
            <td class="text-dim fs-sm">${esc(u.created_at || '')}</td>
  ...
  function linkedUserBannerHTML(tab) {
    ...
    return `<div class="flex items-center gap-8" style="margin-bottom:8px;padding:4px 8px;background:var(--bg-input);border:1px solid var(--border);border-radius:4px;font-size:0.8rem">
      ...
      <div style="padding:12px">
        <div class="flex justify-between items-center mb-16">
          <div>
            ...
            <div class="text-dim fs-md">${esc(u.email || 'no email')}</div>
          </div>
          <span class="${u.tier === 'pro' ? 'text-ok' : 'text-dim'}" style="font-size:0.9rem">${esc(u.tier)}</span>
        </div>

        <div class="rsl-authdb-stats mb-16">
          ...
          <div class="rsl-authdb-stat-card cursor-pointer" data-view-linked="tokens"><div class="rsl-authdb-stat-value">${u.tokenCount}</div><div class="rsl-authdb-stat-label">tokens</div></div>
          <div class="rsl-authdb-stat-card cursor-pointer" data-view-linked="usage-logs"><div class="rsl-authdb-stat-value">${u.usageCount}</div><div class="rsl-authdb-stat-label">usage logs</div></div>
          <div class="rsl-authdb-stat-card cursor-pointer" data-view-linked="receipts"><div class="rsl-authdb-stat-value">${u.receiptCount}</div><div class="rsl-authdb-stat-label">receipts</div></div>
          ...
          <div class="rsl-authdb-stat-card cursor-pointer" data-view-linked="payments"><div class="rsl-authdb-stat-value">${u.paymentEventCount || 0}</div><div class="rsl-authdb-stat-label">payments</div></div>
        </div>
        ...
        <div style="margin-bottom:16px;padding:8px 12px;border:1px solid var(--border);border-radius:4px">
          ...
          ${u.customer_id ? `<div class="text-dim fs-sm">customer: ${esc(u.customer_id)}</div>` : ''}
          ${u.subscription_id ? `<div class="text-dim fs-sm">sub id: ${esc(u.subscription_id)}</div>` : ''}
        </div>` : ''}
        ...
        <div class="mb-16">
          ...
          <table class="table-console">
            ...
            ${u.socialAccounts.map(sa => `<tr>
              ...
              <td class="text-dim fs-sm">${esc(sa.provider_user_id)}</td>
              <td class="text-dim fs-sm">${esc(sa.created_at)}</td>
        ...
        <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:16px">
          ...
          <div class="form-group"><label class="form-label">email</label><input type="email" class="form-input fs-md" id="rsl-adb-edit-email" value="${esc(u.email || '')}"></div>
          <div class="form-group"><label class="form-label">name</label><input type="text" class="form-input fs-md" id="rsl-adb-edit-name" value="${esc(u.name || '')}"></div>
          <div class="form-group"><label class="form-label">tier</label><select class="form-input fs-md" id="rsl-adb-edit-tier">
          ...
          <div class="form-group"><label class="form-label">avatar url</label><input type="text" class="form-input fs-md" id="rsl-adb-edit-avatar" value="${esc(u.avatar_url || '')}"></div>
          <div class="form-group"><label class="form-label">prepaid credits</label><input type="number" class="form-input fs-md" id="rsl-adb-edit-credits" value="${u.prepaid_credits || 0}" min="0" step="1"></div>
          <div class="form-group"><label class="flex gap-8 items-center fs-md">
          ...
          <div style="border-top:1px solid var(--border);padding-top:8px;margin-top:4px">
            ...
            <div class="form-group"><label class="form-label">subscription status</label><input type="text" class="form-input fs-md" id="rsl-adb-edit-sub-status" value="${esc(u.subscription_status || '')}" placeholder="active, cancelled, expired..."></div>
            <div class="form-group"><label class="form-label">subscription product</label><input type="text" class="form-input fs-md" id="rsl-adb-edit-sub-product" value="${esc(u.subscription_product || '')}" placeholder="pro-unlimited, etc."></div>
            <div class="form-group"><label class="form-label">customer id</label><input type="text" class="form-input fs-md" id="rsl-adb-edit-customer-id" value="${esc(u.customer_id || '')}"></div>
            <div class="form-group"><label class="flex gap-8 items-center fs-md">
        ...
        <div style="border-top:1px solid var(--border);padding-top:12px">
          ...
          <div id="rsl-adb-delete-area" class="hidden mt-8">
            <span class="text-dim fs-sm">type DELETE USER to confirm:</span>
  ...
  function renderTokensTable() {
    ...
    el.innerHTML = `
      ...
      <div id="rsl-adb-purge-tokens-area" class="hidden" style="margin-bottom:12px;padding:8px 12px;border:1px solid var(--border)">
        <span class="text-dim fs-md">type PURGE to confirm:</span>
        ...
        <div id="rsl-adb-purge-tokens-result" class="mt-8 fs-md"></div>
      </div>
        ...
        <table class="table-console">
          ...
          ${tokens.map(t => {
            ...
            return `<tr>
              ...
              <td class="text-dim fs-sm">${esc(t.expires_at)}</td>
              <td>${status}</td>
              <td class="text-dim fs-sm">${esc(t.created_at)}</td>
    ...
    el.innerHTML = `
      ...
      <div class="flex justify-between items-center flex-wrap gap-8 mb-12">
        <div class="flex gap-8 items-center flex-wrap">
          ...
          <span class="text-dim fs-md">action:</span>
      ...
      <div id="rsl-adb-purge-usage-area" class="hidden" style="margin-bottom:12px;padding:8px 12px;border:1px solid var(--border)">
        <div class="flex gap-8 items-center flex-wrap">
          <span class="text-dim fs-md">delete logs older than</span>
          ...
          <span class="text-dim fs-md">days</span>
        </div>
        <div class="flex gap-8 items-center mt-8">
          <span class="text-dim fs-md">type PURGE to confirm:</span>
        ...
        <div id="rsl-adb-purge-usage-result" class="mt-8 fs-md"></div>
      </div>
      ...
      <div class="scroll-x">
        <table class="table-console">
          ...
          ${logs.map(l => {
            ...
            return `<tr>
            <td class="fs-md">${esc(userLabel)}</td>
            <td class="text-dim fs-xs" title="${esc(l.device_id || '')}">${esc(device)}</td>
            <td class="text-dim fs-sm">${esc(l.ip_address)}</td>
            <td>${esc(l.action)}</td>
            <td class="text-dim">${l.credits_used != null ? l.credits_used : '--'}</td>
            <td class="text-dim fs-xs" title="${esc(l.user_agent || '')}">${esc(agent)}</td>
            <td class="text-dim fs-xs" title="${esc(l.details || '')}">${esc(detail)}</td>
            <td class="text-dim fs-sm">${esc(l.created_at)}</td>
          </tr>`;}).join('')}
        </table>
      </div>
      ...
      <div class="scroll-x">
        <table class="table-console">
          ...
          ${receipts.map(r => {
            ...
            const userClick = r.user_id ? `data-user-id="${esc(r.user_id)}" style="cursor:pointer;text-decoration:underline;text-decoration-style:dotted;font-size:0.85rem"` : 'class="fs-md"';
            return `<tr>
              <td ${userClick}>${esc(r.user_name || r.user_email || r.user_id?.slice(0, 8) + '...')}</td>
              <td><span class="${statusClass} fs-sm">${esc(r.status || '--')}</span></td>
              <td class="fs-md">${esc(r.merchant_name || '--')}</td>
              <td class="text-dim fs-sm">${esc(amountStr)}</td>
              <td class="text-dim fs-sm">${esc(shortModel(r.model_used))}</td>
              <td class="text-dim fs-sm">${timeStr}</td>
              <td class="fs-sm">${esc(r.category || '--')}</td>
              <td class="text-dim fs-sm">${esc(r.scanned_at || '')}</td>
              <td>
  ...
  function renderReceiptDetail() {
    ...
    body.innerHTML = `
      <div style="padding:12px">
        <div class="flex justify-between items-center mb-16">
          <div>
            <div class="text-dim fs-md">${esc(r.user_name || r.user_email || 'unknown')}</div>
            <div class="text-dim fs-sm">${esc(shortModel(r.model_used))} | ${timeStr}</div>
          </div>
        ...
        <div style="margin-bottom:16px;padding:8px 12px;border:1px solid var(--border);border-radius:4px">
          ...
          <div class="text-err fs-md">${esc(r.error_message)}</div>
        </div>` : ''}

        <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:16px">
          <div class="text-dim" style="font-size:0.8rem;margin-bottom:4px">receipt data</div>
          <pre style="font-size:0.8rem;background:var(--bg-input);padding:8px;border-radius:4px;overflow-x:auto;max-height:40vh;overflow-y:auto;border:1px solid var(--border)">${esc(JSON.stringify(parsedData, null, 2))}</pre>
        </div>

        <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:16px">
          <div class="text-dim" style="font-size:0.8rem;margin-bottom:8px">edit</div>
          <div class="form-group"><label class="form-label">status</label><select class="form-input fs-md" id="rsl-receipt-edit-status">
          ...
          <div class="form-group"><label class="form-label">merchant name</label><input type="text" class="form-input fs-md" id="rsl-receipt-edit-merchant" value="${esc(r.merchant_name || '')}"></div>
          <div class="flex gap-8">
          ...
          <div class="form-group"><label class="form-label">category</label><input type="text" class="form-input fs-md" id="rsl-receipt-edit-category" value="${esc(r.category || '')}" placeholder="grocery, restaurant, utility..."></div>
          <div class="form-group"><label class="form-label">tags</label><input type="text" class="form-input fs-md" id="rsl-receipt-edit-tags" value="${esc(r.tags || '')}" placeholder="comma-separated tags"></div>
          <div class="form-group"><label class="form-label">source</label><input type="text" class="form-input fs-md" id="rsl-receipt-edit-source" value="${esc(r.source || '')}"></div>
        ...
        <div style="border-top:1px solid var(--border);padding-top:12px">
          ...
          <div id="rsl-receipt-delete-area" class="hidden mt-8">
            <span class="text-dim fs-sm">type DELETE RECEIPT to confirm:</span>
  ...
  function renderPaymentsTable() {
    ...
    el.innerHTML = `
      ...
      <div class="flex justify-between items-center flex-wrap gap-8 mb-12">
        <div class="flex gap-8 items-center flex-wrap">
          <span class="text-dim fs-md">event:</span>
          ...
          <span class="text-dim fs-md">status:</span>
      ...
      <div id="rsl-adb-purge-payments-area" class="hidden" style="margin-bottom:12px;padding:8px 12px;border:1px solid var(--border)">
        <div class="flex gap-8 items-center flex-wrap">
          <span class="text-dim fs-md">delete events older than</span>
        ...
        <div id="rsl-adb-purge-payments-result" class="mt-8 fs-md"></div>
      </div>
      ...
      <div class="scroll-x">
        <table class="table-console">
          ...
          ${payments.map(p => {
            ...
            const userClick = p.user_id ? `data-user-id="${esc(p.user_id)}" style="cursor:pointer;text-decoration:underline;text-decoration-style:dotted;font-size:0.85rem"` : 'class="fs-md"';
            return `<tr>
              <td ${userClick}>${esc(userLabel)}</td>
              <td class="fs-md">${esc(p.event_type)}</td>
              <td class="text-dim fs-sm" title="${esc(p.polar_id || '')}">${esc(polarId)}</td>
              <td class="text-dim fs-sm">${esc(p.source || '--')}</td>
              <td><span class="${statusClass} fs-sm">${esc(p.status || '--')}</span></td>
              <td class="text-dim fs-sm">${esc(p.created_at || '')}</td>
              <td>
                <div class="flex gap-4">
        ...
        <div style="padding:12px">
          <div class="flex justify-between items-center mb-16">
            <div>
              ...
              <div class="text-dim fs-md">${esc(p.user_name || p.user_email || 'unknown user')}</div>
            </div>
          ...
          <div style="border-top:1px solid var(--border);padding-top:12px">
            ...
            <pre style="font-size:0.8rem;background:var(--bg-input);padding:8px;border-radius:4px;overflow-x:auto;max-height:50vh;overflow-y:auto;border:1px solid var(--border)">${esc(JSON.stringify(payload, null, 2))}</pre>
          </div>
          ...
          <div style="border-top:1px solid var(--border);padding-top:12px;margin-top:16px">
            ...
            <div id="rsl-payment-delete-area" class="hidden mt-8">
              <span class="text-dim fs-sm">type DELETE EVENT to confirm:</span>
  ...
  function paginationHTML(current, totalPages, total, pageBtnClass) {
    ...
    return `<div class="flex justify-between items-center" style="padding:8px 12px;border-top:1px solid var(--border)">
      <span class="text-dim fs-sm">page ${current} of ${totalPages} (${total} total)</span>
