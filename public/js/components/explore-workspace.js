...
const ExploreWorkspaceComponent = (() => {
  ...
  function createTextareaForTab(tab) {
    ...
    ta.style.display = 'none';   /* .nw-textarea class carries the styling; shown via .style.display toggle */
  ...
  function restoreTabState(idx) {
    ...
    document.getElementById('nw-status-protected').style.color = tab.protected ? 'var(--warning)' : '';
            ...
            <div id="nw-main" class="nw-main">
              ...
              <div id="nw-editor-area" style="flex:1;display:flex;flex-direction:column;min-width:0">
                ...
                <div id="nw-editor-toolbar" class="nw-editor-toolbar" style="padding:4px 10px;border-bottom:1px solid var(--border);font-size:11px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
                  ...
                  <span id="nw-dirty-badge" style="color:var(--accent-blue);font-weight:bold;display:none">*</span>
  ...
  function renderBreadcrumb() {
    ...
    let html = `<a href="#" class="nw-bc-link" data-path="" style="color:var(--accent-blue)">root</a>`;
    let acc = '';
    for (const p of parts) {
      acc += (acc ? '/' : '') + p;
      html += ` / <a href="#" class="nw-bc-link" data-path="${esc(acc)}" style="color:var(--accent-blue)">${esc(p)}</a>`;
    }
    el.innerHTML = html;
  ...
  function renderFileList(items) {
    ...
    for (const item of items) {
      ...
      const protBadge = item.protected ? '<span style="color:var(--warning);font-size:9px;margin-left:4px" title="protected">&#9899;</span>' : '';
      const lockBadge = item.locked ? '<span style="color:var(--danger);font-size:9px;margin-left:4px" title="locked">&#128274;</span>' : '';
      ...
      const deleteBtn = canDelete ? `<button class="nw-item-delete-btn" data-path="${esc(item.path)}" data-name="${esc(item.name)}" data-isdir="${item.isDir ? 1 : 0}" style="margin-left:auto;padding:0 4px;background:none;border:none;color:var(--text-dim);cursor:pointer;font-size:11px;opacity:0.6" title="Delete">&#10005;</button>` : '';
      const isOpen = openPaths.has(item.path);
      const isActive = isOpen && item.path === curPath;
      const activeStyle = isActive ? 'background:var(--bg-tertiary);' : isOpen ? 'background:rgba(255,255,255,0.015);' : '';
